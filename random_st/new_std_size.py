...
from model1.m_std_05 import Modelstd05
from model1.m_std_15 import Modelstd15
...
### Std 05인 경우
results = {
    "R2_Std_05": [],      # bce_binary * mse (최종)
    "R2_Std_15": [],     # bce_prob * mse (sigmoid 가중)
}
for i in np.random.randint(1,100,size = 20):
    ...
    model = Modelstd05(x_dim,x2_dim,c_dim, z_dim=8,z2_dim = 8).to(device)
    ...
    ##r2저장
    results["R2_Std_05"].append(float(r2_bce_mse_sig))
...
for i in np.random.randint(1,100,size = 20):
    ...
    r2_bce_mse_sig = r2_score(x_true.flatten(),final_x_sig.flatten())
    ##각각의 r2저장
    results["R2_Std_15"].append(float(r2_bce_mse_sig))

save_path = "./results_Std_Size.json"
